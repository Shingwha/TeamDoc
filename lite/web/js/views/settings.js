...
(function () {
  ...
  /** 个人设置作用域(#/settings):账号(只读)+ 修改密码 + 访问令牌 */
  window.Views.settingsLevel = function () {
    return {
      key: 'settings',
      render(ctx) {
        const frag = Scope.html(
          ...
          '<div class="stack">' +
          // 账号是壳里已有的事实:App.user 就是 /api/auth/me 的 user 段(启动时探过),
          // 这一页只是把它显示出来 —— 再问一次服务端得到的还是同一个对象
          UI.card({
            title: '账号', icon: 'user-3-line',
            body: accountBody(App.user || {}),
          }) +
          // 修改密码
          UI.card({
          ...
          UI.card({
            ...
            body: '<div id="pat-list">' + UI.skeleton('rows', 3) + '</div>',
          }) +
          '</div>' +
          '</div>');

        // ---------- 修改密码 ----------
        frag.querySelector('#pwd-form').addEventListener('submit', async (e) => {
          e.preventDefault();
        ...
        // ---------- PAT 管理 ----------
        const patList = frag.querySelector('#pat-list');
        function loadPats() {
        ...
        frag.querySelector('#pat-new').onclick = () => {
          UI.formModal({
            title: '新建访问令牌',
            okText: '创建',
            fields: [
  ...
  /** 账号卡片内容。id 是唯一不变的标识(邮箱是可改的登录名) */
  function accountBody(u) {
    return '<div class="field"><label>邮箱</label>' +
