...
### 升级到「手动保存 + 正文写入带基线」版本(2026-09)

无表结构变更,正常替换代码重启即可。但用户能感觉到的行为变了两处,升级前值得说一声:

- **保存改成手动了**:编辑器不再自动保存,写完要点「保存」或按 `Ctrl/Cmd+S`;没保存就切文档/关标签
  会被拦下来问一句。**这会改变所有人的使用习惯**,建议提前知会。
- **两人同编不再静默覆盖**:服务端发现保存所基于的版本已经过期,会回 409 并把冲突交给用户,
  由他在差异里逐块选择保留哪边(此前是后写者整篇覆盖、先写者静默丢字,而两边都显示"已保存 ✓")。

服务端侧的落实要点:

- `docs.version` 早就存在,不新增列、不动数据,也不需要任何手工 SQL。
- **`PUT /api/docs/{id}/content` 的 `baseVersion` 只对浏览器(web 会话)必填**;PAT 客户端
  (`td` CLI、自建脚本、`td api`)**缺省即无条件覆盖,行为与升级前完全一致,不需要改代码或重装**。
  显式带上 `baseVersion` 就一律照查(先 `GET /api/docs/{id}` 取 `version`);只是追加内容用 `/append`。
- 升级瞬间**仍开着的旧页面**在保存时会失败(它不带基线,而 web 会话必须带):刷新即可。这是有意为之
  ——不接受"没声明基线就整篇覆盖"的旧行为继续悄悄生效。
- `VERSION_MERGE_MINUTES` 默认由 `5` 改为 `0`:保存变手动后,每次保存都该留下一个可回退的还原点。
  想保留过去那种稀疏历史,在 `.env` 里显式设回 `5` 即可。
