...
> 面向把这套系统放到内网服务器上长期运行的人。开发相关约定见 `ARCHITECTURE.md`。
...
| `BACKUP_KEEP` | `7` | 每个目标目录保留最近几份备份,更旧的自动删除 |
| `VERSION_MERGE_MINUTES` | `0` | 同一人在窗口内的连续保存合并为一个还原点。默认 0 = 每次保存都留一版;想要稀疏历史可调大 |
| `ZIP_MAX_BYTES_MB` | `4096` | 单次打包下载的总字节上限 |
| `WATCHDOG_STALL_SECONDS` | `10` | 事件循环卡顿超过它就把线程栈转储到 `logs/` |

**全部环境变量只在一个地方读取与校验:`config.py`。** 写错的值(比如 `MAX_UPLOAD_MB=abc`)
会让服务**启动即失败**并指出是哪个变量,而不是悄悄退回默认值 —— 后者最坏的情况是
"以为设了上限,其实没有"。当前生效值可在「管理后台 → 存储」只读查看。
...
**只能恢复到结构相同的版本。** 结构的唯一来源是 `models.py`,启动时只建缺失的表、
已有表不加列也不改列 —— 所以上传备份时会用同一份结构判定(`schema.check_drift`)先比对:
缺列、多列、**列类型不同的备份一律当场拒绝**,而不是等重启后起不来。
...
| 上传备份被拒"结构与当前版本不一致" | 备份来自结构不同的版本(缺列 / 多列 / 列类型不同)。用生成该备份的那一版代码恢复 |
...
这是**结构漂移**:库与 `models.py` 对不上(库里多列/少列/列类型不同)。启动自检逐列比对,
发现后**拒绝启动**并在日志里点名是哪一列、模型要求什么 —— 名字相同而类型不同是最隐蔽的一种
(读能过,直到按值匹配或写入时才炸),所以这里不给它机会。

本项目**不做结构迁移**:结构与 `models.py` 不一致的库不能使用,处理方式只有一种 ——

- 库里的内容不需要保住:停服,删掉报错信息里那个库文件(默认 `data/teamdoc.db`),
  重启即按 `models.py` 一次建全。
- 需要保住内容:先用**对应版本的旧代码**把服务跑起来,把内容导出(文档正文、云空间文件、
  成员列表),再切回新版本、重建库、导进去。

### 升级前的兼容性说明

- **不支持把代码回退到旧版本**:新版本可能已写入旧代码不认识的字段。要回退请连同对应时点的
  备份一起恢复。
- API 契约(错误码、字段面、引用语法)见 `ARCHITECTURE.md`。若有外部脚本调用 API,
  升级前先对一下那里;`baseVersion` 规则要看「文档写入」那一节。
...
- 不再为更老的内核做专门兜底:媒体查询监听走标准的 `matchMedia().addEventListener`
  (Chrome 14+ / Safari 14+ / Firefox 6+ 即有),已废弃的 `addListener` 路径不再保留。
...
超过这个规模(比如用户上百、单目录上万文件、文档上万篇)需要先做的改动见 `ARCHITECTURE.md` 的「已知边界」。
