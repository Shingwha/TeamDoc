...
| `VERSION_MERGE_MINUTES` | `0` | 同一人在窗口内的连续保存合并为一个版本还原点。默认 0 = 不合并(保存已改为手动,每次保存都留一个还原点);想要稀疏历史可调大 |
...
**只能恢复到结构相同的版本。** 本项目的结构唯一来源是 `models.py`,**没有数据库迁移机制**:
启动时只建缺失的表、已有表不加列/改列,`models.py` 与库对不上会**直接启动失败**。所以上传
备份时会用同一份结构判定(`schema.check_drift`)预先比对:缺列、多列、**列类型不同的备份
一律当场拒绝**,而不是等重启后起不来。跨版本请用同版本代码恢复后再升级。

> 换句话说:**升级到结构有变的新版本时,数据目录要重建**(旧库不会被就地改造)。
> 需要保住的内容请先用旧版本把服务跑起来导出,再导进新库。
...
| 上传备份被拒"结构与当前版本不一致" | 备份来自结构不同的版本(缺列 / 多列 / 列类型不同)。用对应版本的代码恢复,或按 §3.3 重建数据目录 |
...
日志落在**数据目录**的 `logs/` 下(`TEAMDOC_DATA_DIR/logs`,默认 `lite/server/data/logs/`,
跟实例走;控制台直跑时也一样,重启不丢):
...
> Windows 控制台直跑时注意:鼠标选中控制台窗口的文字会暂停**该窗口的显示**(快速编辑
> 模式),按 Esc 或回车恢复。服务本身不受影响 —— 日志写出在独立线程上、只入队不阻塞
> 事件循环(见 logsetup);这条曾经能把整个服务冻死(2026-09-12 事故)。
> 正式部署仍建议用 §2.1 的服务方式,由 NSSM/systemd 落盘。
...
3. 重启服务
4. 跑一遍页面资产校验,确认前端资源完整(零外链 + no-cache,KaTeX/字体、Prism 语言包、
   Mermaid 图表库全量可达;内网部署前后都该跑):
...
没有迁移机制(见本章开头),所以**结构变了自己补** —— 漏补的后果是启动时被漂移自检拦下并给出修复指引,不会带病运行。

| 版本(2026-09 起) | 结构变化 | 处理 |
|---|---|---|
| 登录节流 + 登录状态与审计 | 新增 `throttle_state`、`login_events`;`sessions` 加 `ip`/`user_agent`/`last_seen_at` | 新表由 `create_all` 自动建;三条 ALTER 见下 |
| 手动保存 + 正文写入带基线 | **无表结构变化**(契约与前端行为变了) | 直接重启;升级瞬间开着的旧标签页需刷新 |
| 历史版本记录带成因 | `doc_versions.label` → `kind`(只描述成因,不再存展示用词) | 三行 SQL 见下,或删库重建 |

**登录节流那版**(新增两表 + 会话加三列),想保留现有会话时执行:

```bash
# 停服后执行(旧会话的 ip/UA 为空,界面显示"未知"或"—")
cd lite/server
.venv/Scripts/python.exe -c "
import sqlite3
c = sqlite3.connect('data/teamdoc.db')
...
- 也可以最省事地 `DROP TABLE sessions;` —— 代价是所有人重新登录一次(会话不承载业务数据)。
- **升级前的备份仍然可以恢复**:这两张表标了"可丢弃状态"(`models.py` 的 `info.disposable`),恢复校验不要求备份里存在它们,启动时会自动补建。
- 新增的环境变量都有默认值(§1.3),不配也能跑;`LOGIN_IP_MAX_FAILS=0` 可关掉来源维度。

**历史版本那版**(`label` → `kind`):`label` 存的是展示用词(覆盖前 / 自动),而"自动"其实只是传输方式的泄漏;改成只描述成因的两档 `save` / `restore`,展示用词归还界面。想留住旧历史就执行:

```bash
.venv/Scripts/python.exe -c "
import sqlite3
c = sqlite3.connect('data/teamdoc.db')
c.execute('ALTER TABLE doc_versions RENAME COLUMN label TO kind')
c.execute(\"UPDATE doc_versions SET kind = 'restore' WHERE kind = '还原前'\")
c.execute(\"UPDATE doc_versions SET kind = 'save' WHERE kind NOT IN ('save', 'restore')\")
c.commit()"
```

不留旧历史的话,直接删数据目录重建更省事(见本章开头)。

**手动保存那版**(无结构变化,但用户能感觉到):升级前建议知会一声 —— ①**保存改成手动**(写完要点「保存」或 `Ctrl/Cmd+S`,没保存就切走会被拦下来问一句);②两人同编不再静默覆盖(保存所基于的版本已过期就回 409,由用户在差异里逐块选择保留哪边)。

- `PUT /api/docs/{id}/content` 的 `baseVersion` **只对浏览器(web 会话)必填**;PAT 客户端(`td` CLI、自建脚本、`td api`)**缺省即无条件覆盖,行为与升级前完全一致,不需要改代码或重装**。显式带上就一律照查(先 `GET /api/docs/{id}` 取 `version`);只是追加内容用 `/append`。
- 升级瞬间仍开着的旧页面在保存时会失败(它不带基线,而 web 会话必须带):刷新即可 —— 有意为之,不接受"没声明基线就整篇覆盖"的旧行为继续悄悄生效。
- `VERSION_MERGE_MINUTES` 默认由 `5` 改为 `0`:保存变手动后,每次保存都该留下一个可回退的还原点。想保留过去那种稀疏历史,在 `.env` 里显式设回 `5`。
