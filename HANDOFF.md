...
- `GET /api/users/directory`(任意登录用户):同事目录,只返回 `id/name/email/avatarColor`,**不含 isAdmin/isDisabled/createdAt**(那是管理后台的字段面),禁用账号不出现。
- **成员添加(批量)**:`UI.personPicker({multi:true, roleSelect})` 候选态勾选(行内角色下拉,默认取 roleSelect.value,每人可单独改),确认后统一以各自角色加入;无手输邮箱框 —— 未注册邮箱服务端必拒,目录搜索已覆盖全部可加对象。前端 `UI.personPicker` 用它做成员选择器(取代让用户手打同事邮箱)。
