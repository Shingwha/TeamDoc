...
| `visual_sweep.py` | 真实 app.js 逐页巡检,收集 onerror/console.error |
...
- 文本预览无大小截断(几十 MB 文件会卡浏览器);备份包含孤儿文件(体积略大,不值得剔除);不做增量备份;恢复只支持整站覆盖,不支持挑单文件取回、不支持回退旧版本代码。
- 公开项目的成员列表(含邮箱)对所有登录用户可见——与同事目录可见面一致,30 人内网可接受;公开是全实例级,无部门/小组范围控制。
